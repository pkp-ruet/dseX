...
- **Marketing landing** (logged-out) — rebuilt from scratch 2026-07-29. **The page has ONE job: look like the most credible stock platform in Bangladesh.** That constrains everything else, so before changing it read the rules below.
  ...
  - **Seven blocks, one job each:** 1 `LandingHero` (English headline + Bengali line + `StockLookup` + `MiniReport` — a real report for a familiar company already on screen; the lookup filters the in-memory `/api/scores` payload so results appear on the keystroke, no request) → 2 `TrustStrip` (**thin** one-row strip: companies scored, sectors covered, live signup count, star average, plus a single "no tips, no rumours / where the data comes from" line — no paragraphs) → 3 `CoreFeatures` (**the promoted block** — Rankings w/ real `LiveRankingPreview`, Portfolio, Watchlist, Alerts in a 2×2 grid; title + one line + visual each, `No login needed` / `Free account` tag per card) → 4 `LiveToday` (opens with **one `MarketMoodLine`** — the backend mood sentence + its Bengali line as a single tappable card to `/market-analysis`, CTA phrased as the question "Is the market up or down?"; added 2026-09-12 because that page was otherwise one small text link away. It is a sentence with **no number in it**, which is why it is allowed where the `MarketTodayCard` index band was cut 2026-08-30 — index levels mean little to a first-time visitor. Fetched via `getMarketState(3600)` so the landing's ISR cadence stays hourly. Then the three standouts from `pickStoryStocks`; `/dse-today` is linked from the block) → 5 `WaysToFind` (the 3-question picker via `FindMyStocks` leads, wrapped in `MotionProvider`; everything else is a plain link row under it — "Is the market up or down today?" / Signals / Lists / A–Z / Compare / Ask, deliberately **not** Rankings since block 3 owns it. Market analysis is phrased as its question there, never as the category label. The 2026-08-30 cut turned the 3 door cards into links and dropped Daily tips / Trending / Most traded from the row: nine competing choices became one picker + five links) → 6 `ReportAnatomy` (the 8 sections really on `/stock/[code]` as a compact two-column list + one link to the hero company's page — the live example panel was cut 2026-08-30; the hero's `MiniReport` already shows a real report, one proof per page) → 7 `StartFromZero` (Bengali `/blog` entry points) + `LandingClose` (admin-approved review quotes + one ask).
...
│   ├── LiveToday.tsx                                       — block 4 (MarketMoodLine → /market-analysis, then standouts = home/StandoutCard)
