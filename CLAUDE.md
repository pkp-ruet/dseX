...
- **Personalized dashboard** (`PersonalizedHome`, logged-in): `DailyBriefing` onboarding checklist (only while setup incomplete — two steps, build-watchlist + personalize; adding a portfolio is sold by the money hero itself, so it's not a step). Then a bento (main column + "Explore" aside on desktop; single column on mobile). **Visual system (2026-09-06 consistency pass — keep it):** every card on the dashboard uses **one header component, `personalized/DashHeader.tsx`** (small uppercase title · optional `HeaderChip`s · one quiet right-hand link), and **one icon set, `personalized/DashIcons.tsx`** (stroked SVG, `currentColor`) — no emoji anywhere on the dashboard (they rendered differently on every phone; medals/rockets/coins were all replaced). Small text uses exactly two sizes, `text-[0.68rem]` for labels/chips and `text-[0.75rem]` for secondary lines — never smaller. Every tappable row pairs `hover:` with an `active:` state (touch never fires hover). Chapter labels outside cards use the quiet `SectionLabel`. **Chapter 1 — Your money** (money-first so a returning user's value is above the fold): the money hero with `HeroGreeting` (date + time-of-day greeting + live `MarketStatusPill` + streak only — the follow-count was dropped, `MyStocksToday`'s "View all N" already carries it) folded into its top — `MoneyHero` (portfolio value shown **immediately on first paint**, tweened only when a refetch changes it — no count-up from 0; today's ৳/% move + beating/trailing-DSEX chip + total-P/L chip + A–F grade via `analyzePortfolio`), `MoneyHeroGhost` when no holdings, `MoneyHeroSkeleton` while the portfolio fetch is in flight → `AttentionStrip` (**the single home for "what happened on your stocks today"** — an alerts list from `buildHomeAlerts` (each `HomeAlert` carries a `kind` mapped to an SVG; the header is neutral — no amber tile / red badge, since most rows are dividends and 52w highs, each row carries its own tone), else one calm daily-brief line from `buildDailyBrief` — its whole-market clause uses the backend mood and is a link to `/market-analysis`; `BriefSegment.href` is how a run becomes a link) → `MyStocksToday` (holdings ∪ watchlist merged, sorted by |move|, 52w/dividend chips, ▲up/▼down day pulse in its header; the H/★ owner tags render **only when the user has both** a portfolio and a watchlist — otherwise every row would wear the same tag) → `SearchBar` (moved here 2026-09-06 so the money chapter reads value → attention → your stocks without a search box interrupting it) → `NewsPeek` (stacked latest-3 headlines, no auto-rotate; footer link goes to `/watchlist` when the user has one, else `/todays-news`, labelled to match) → `InstallHomeBanner`. **Chapter 2 — Ideas for you**: `IdeasCard` — **one plain card** with its own `DashHeader` (title "Your ideas today", or "Ideas for you today" when untuned; date + "N new" chips fed by `daily_picks.new_codes`; per-tab "See all" + Tune on the right) and a **Picks / Buys / Tips segmented control** (only non-empty tabs, last tab in localStorage `dsex.home.ideasTab`, 3 rows per tab, numbered badges for picks). The earlier cut stacked three tinted panels with medals + coloured spines — the heaviest block on the page — **do not bring that back**. **Explore aside** (`lg:sticky` so the right column doesn't end two screens early) under a quiet `SectionLabel`: `MarketTodayCard` (**the front door to `/market-analysis`** since 2026-09-12: the headline is the backend mood from `marketState.mood` — falling back to a breadth read only while the bundle loads — so the card and the page never disagree; the one header link is "Full picture" → `/market-analysis`; the DSEX/DSES/DS30 row is the link to `/dse-today`; breadth bar; four tiles — cheap? / healthy companies → market analysis, dividends → `/dividend-calendar`, turnover → DSE Today; and a "Since yesterday" line from `since_yesterday` (breadth rank → healthy delta → index move) that links through. The always-green pulsing "live" dot and the footer button stay cut, the greeting's `MarketStatusPill` is the one honest live cue), `ExploreLinks` (flat tappable rows out to Rankings / Market Analysis / Trending / Ready-made lists / Browse A–Z / বাংলা ব্লগ — no preview tables, the full pages are one tap away; replaced `DiscoverCard` + `CoreFeatureTiles` 2026-08-30, which also dropped the dashboard's `getTop20` fetch). `StoriesCard` ("Three worth knowing today") was dropped from the dashboard the same day — it competed with `IdeasCard` as a third ideas surface; the three standouts still run on the landing page (block 4).
...
│   │   │                            Market Analysis / Trending / Lists / Browse A–Z / বাংলা ব্লগ), no previews
