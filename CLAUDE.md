...
- **Personalized dashboard** (`PersonalizedHome`, logged-in) — **rebuilt 2026-09-16** for a reader who is weak in English and has never seen a P/E. Four rules came out of that rebuild; keep them:
  ...
  - **Order (single column on a phone):** money hero (brief + value + grade; `MoneyHeroGhost` when no holdings, `MoneyHeroSkeleton` while the bundle is in flight) → `MyStocksToday` (holdings ∪ watchlist by |move|, **company name under the code**, chips: 52w high/low · dividend soon · `Alert at ৳X` for an armed price alert · `Full report` when a deep-analysis report exists; H/★ tags only when the user has both) → `AttentionStrip` (**only** targets hit, Buy More / **Sell** flips — Sell was hidden before — and dividends; for a holding the dividend row says what they will be paid, `GP pays you ৳1,200 · record date …`, from `dividend_cash` × qty. Price moves / 52w rows are NOT here, they are chips above — nothing is listed twice) → `NewsPeek` → `SearchBar` (**desktop only**: the navbar pill + bottom-bar Search already cover phones) → `InstallHomeBanner` → `TodaysIdeas` → `DailyBriefing` setup checklist (**moved below the money chapter** — a returning user with a portfolio met "Finish setting up" before their own money) → `StartHereCard` (three beginner guides from `lib/blog-bn.ts`, only for an account with neither portfolio nor watchlist). **Explore aside** (`lg:sticky`): `MarketTodayCard` (headline = backend mood; in বাংলা the "Full picture" link and tiles go to `/share-bazar`, the Bengali daily article, instead of `/market-analysis`) → `ExploreLinks` (ten rows, five visible + "More": Rankings · আজকের শেয়ার বাজার · Who pays cash soon · Ask TopStock AI · What others are looking at · Market Analysis · Trending · Lists · A–Z · বাংলা ব্লগ).
...
│   │   ├── MarketTodayCard.tsx   — bilingual; বাংলা links to /share-bazar
│   │   ├── ExploreLinks.tsx      — client: ten link rows, five visible + "More"
│   │   ├── StartHereCard.tsx     — three beginner guides from the Bengali blog, brand-new accounts only
