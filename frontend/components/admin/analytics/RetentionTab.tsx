    ...
    <div className="flex flex-col gap-6">
      ...
      <div>
        <div className="flex items-baseline justify-between gap-2 mb-2">
          <h3 className="text-sm font-bold text-text-main">New-user retention</h3>
          <span className="text-xs text-text-muted">still active N+ days after joining</span>
        </div>
        {r ? (
          <MetricRow metrics={retentionTiles} cols="grid-cols-1 sm:grid-cols-3" />
        ) : (
          <p className="text-text-muted text-sm">Loading retention…</p>
        )}
      ...
      <Panel title="Weekly cohorts" note="% of each signup week active, by weeks since joining">
        {r ? <CohortGrid rows={r.cohort_grid} /> : <p className="py-6 text-center text-xs text-text-muted">Loading…</p>}
      </Panel>
      ...
      >
        {r ? <HoursHeatmap matrix={r.active_hours.matrix} max={r.active_hours.max} /> : <p className="py-6 text-center text-xs text-text-muted">Loading…</p>}
            ...
            return (
              <div key={seg}>
                <div className="flex items-center justify-between text-xs mb-1">
                  ...
                  <span className="text-text-muted tabular-nums">{count} · {w.toFixed(0)}%</span>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {SEGMENT_ORDER.map((seg) => {
          ...
          return (
            <div key={seg} className="soft-card rounded-xl overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-border" style={{ borderTop: `3px solid ${m.color}` }}>
                <h3 className="text-sm font-bold text-text-main">{m.label}</h3>
                <span className="text-xs font-semibold tabular-nums" style={{ color: m.color }}>{users.length}</span>
              </div>
              <div className="max-h-72 overflow-y-auto divide-y divide-cell-rule">
                {users.length === 0 && (
                  <p className="px-4 py-6 text-center text-xs text-text-muted">No users.</p>
                )}
                {users.slice(0, 50).map((u) => (
                  <button
                    key={u.user_id}
                    onClick={() => onSelect(u)}
                    className="w-full flex items-center justify-between gap-3 px-4 py-2.5 text-left hover:bg-surface-2 transition-colors"
                  >
                    <span className="min-w-0">
                      <span className="block text-sm font-medium text-text-main truncate">
                        {u.display_name || u.email || u.phone || "No name"}
                      </span>
                      <span className="block text-xs text-text-muted">joined {fmtDate(u.created_at)}</span>
                    </span>
                    <span className="text-xs text-text-muted shrink-0 tabular-nums">{timeAgo(u.last_seen_at)}</span>
                  </button>
                ))}
                {users.length > 50 && (
                  <p className="px-4 py-2 text-center text-xs text-text-muted">+{users.length - 50} more</p>
                )}
              </div>
            </div>
          );
