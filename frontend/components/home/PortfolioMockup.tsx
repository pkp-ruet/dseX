    ...
    <div className="soft-card overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 sm:px-5 py-4 border-b border-border">
        <div className="flex flex-col items-center justify-center w-14 h-14 rounded-xl border-2 shrink-0" style={{ background: "rgba(21,128,61,0.10)", borderColor: "rgba(21,128,61,0.45)", color: "var(--positive)" }}>
          ...
          <span className="text-xs font-bold uppercase tracking-wide">Solid</span>
        </div>
        <div className="min-w-0 flex-1">
          <div className="text-xs font-bold uppercase tracking-[0.14em] text-positive">Portfolio Verdict</div>
          <p className="text-sm leading-snug text-text-main font-medium">
      ...
      {/* Value + P/L */}
      <div className="grid grid-cols-2 gap-3 px-4 sm:px-5 py-3 border-b border-border">
        <div>
          <div className="text-xs font-bold uppercase tracking-wider text-text-muted">Current value</div>
          <div className="text-lg font-extrabold tabular-nums text-text-main">৳64,040</div>
        </div>
        <div>
          <div className="text-xs font-bold uppercase tracking-wider text-text-muted">Total P/L</div>
          <div className="text-lg font-extrabold tabular-nums text-positive">+৳1,340 <span className="text-xs">(+2.1%)</span></div>
        </div>
      </div>
        ...
        {SUBSCORES.map((s) => (
          <div key={s.label} className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between">
              <span className="text-xs font-bold uppercase tracking-wide text-text-main">{s.label}</span>
              <span className="text-sm font-black tabular-nums" style={{ color: s.color }}>
                {s.value.toFixed(1)}<span className="text-xs text-text-muted font-semibold">/10</span>
              </span>
            </div>
            <div className="h-1.5 rounded-full bg-surface-2 overflow-hidden">
              <div className="h-full rounded-full" style={{ width: `${(s.value / 10) * 100}%`, background: s.color }} />
            </div>
            <span className="text-xs text-text-muted leading-tight">{s.hint}</span>
          </div>
        ))}
      </div>

      {/* Insight bullets */}
      <div className="px-4 sm:px-5 pb-4 flex flex-col gap-1.5 border-t border-border pt-3">
        {INSIGHTS.map((it) => {
          const good = it.tone === "good";
          const color = good ? "var(--positive)" : "var(--watch)";
          return (
            <div key={it.text} className="flex items-center gap-2 text-sm text-text-main">
