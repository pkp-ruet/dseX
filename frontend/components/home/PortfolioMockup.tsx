    ...
    <div className="soft-card overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 sm:px-5 py-4 border-b border-[var(--border)]">
        <div className="flex flex-col items-center justify-center w-14 h-14 rounded-xl border-2 shrink-0" style={{ background: "rgba(21,128,61,0.10)", borderColor: "rgba(21,128,61,0.45)", color: "var(--positive)" }}>
          ...
          <span className="text-[0.68rem] font-bold uppercase tracking-wide">Solid</span>
        </div>
        <div className="min-w-0 flex-1">
          <div className="text-[0.68rem] font-bold uppercase tracking-[0.14em] text-[var(--positive)]">Portfolio Verdict</div>
      ...
      <div className="grid grid-cols-2 gap-3 px-4 sm:px-5 py-3 border-b border-[var(--border)]">
        <div>
          <div className="text-[0.68rem] font-bold uppercase tracking-wider text-[var(--text-muted)]">Current value</div>
          <div className="text-lg font-extrabold tabular-nums text-[var(--text)]">৳64,040</div>
        </div>
        <div>
          <div className="text-[0.68rem] font-bold uppercase tracking-wider text-[var(--text-muted)]">Total P/L</div>
      ...
      <div className="grid grid-cols-3 gap-3 px-4 sm:px-5 py-4">
        {SUBSCORES.map((s) => (
          <div key={s.label} className="flex flex-col gap-1.5">
            ...
            <span className="text-[0.68rem] text-[var(--text-muted)] leading-tight">{s.hint}</span>
          </div>
        ))}
      </div>
