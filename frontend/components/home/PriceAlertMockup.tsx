  ...
  return (
    <div className="soft-card overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-surface-2">
        <span className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-[0.12em] text-text-main">
          <span className="text-watch">{BELL}</span> My Alerts
        </span>
        <span className="text-xs text-text-muted">we watch the price for you</span>
      </div>
        ...
        {ROWS.map((r) => {
          ...
          return (
            <div key={r.code} className="flex items-center gap-3 px-4 py-3">
              ...
              {hit ? (
                <span
                  className="text-xs font-bold uppercase tracking-[0.08em] px-2 py-0.5 rounded-full"
