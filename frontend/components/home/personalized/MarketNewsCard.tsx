        ...
        {rows.map((n, i) => {
            ...
            <>
              <span className="min-w-0 flex-1">
                <span className="flex items-center gap-1.5">
                  {code && <span className="ticker-tag ticker-tag--static shrink-0 text-[0.68rem]">{code}</span>}
                  {code && <OwnerMark code={code} held={held} watched={watched} lang={lang} />}
                  {date && <span className="text-[0.68rem] font-medium text-[var(--text-muted)]">{date}</span>}
                </span>
                <span className="mt-1 block text-[0.86rem] font-semibold leading-snug text-[var(--text)] line-clamp-2">
                  {n.title}
                </span>
              </span>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="mt-1 shrink-0 text-[var(--text-muted)]" aria-hidden>
          ...
          return (
            <li key={`${n.trading_code}-${n.post_date}-${i}`}>
              {code ? (
                <Link prefetch={false} href={`/stock/${code}`} className={`${cls} transition-colors hover:bg-[var(--surface-2)] active:bg-[var(--surface-2)]`}>
                  {inner}
                </Link>
              ) : (
